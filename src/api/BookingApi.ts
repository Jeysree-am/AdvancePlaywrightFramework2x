  ...
  // ---------- token lifecycle ----------
    ...
    private async sendAuthed(
      ...
      async getAllBookings(filters?: BookingFilters): Promise<BookingId[]> {
        ...
        return (await this.apiHelper.parseJsonResponse(response)) as BookingId[];
    }
      ...
      async getBooking(id: number): Promise<Booking> {
        ...
        return (await this.apiHelper.parseJsonResponse(response)) as Booking;
    }
     ...
     */
     async auth(username = this.username, password = this.password): Promise<string> {
        const response = await this.apiHelper.post(`${this.baseUrl}/auth`, {
        ...
        const body = await this.apiHelper.parseJsonResponse(response) as { token?: string; reason?: string };
        if (!body.token) {
    ...
    async createBooking(payload: Booking): Promise<CreateBookingResponse> {
        ...
        return (await this.apiHelper.parseJsonResponse(response)) as CreateBookingResponse;
    }
    ...
    async updateBooking(id: number, payload: Booking, token?: string): Promise<Booking> {
     ...
     async patchBooking(id: number, partial: Partial<Booking>, token?: string): Promise<Booking> {
        const response = await this.sendAuthed(
            (t) => this.apiHelper.patch(`${this.baseUrl}/booking/${id}`, {
                data: partial,
                headers: this.authHeaders(t),
            }),
            token,
